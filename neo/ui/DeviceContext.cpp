...
#ifdef _HARMATTAN_PLUS
void idDeviceContext::karinCalePosition(int xi, int yi, float *xr, float *yr)
{
	if(!xr || !yr)
		return;

	float x = (float)xi * (vidWidth / (float)scr_width);
	float y = (float)yi * (vidHeight / (float)scr_height);
