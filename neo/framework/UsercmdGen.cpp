...
class idUsercmdGenLocal : public idUsercmdGen
{
	...
	void karinPollAction(void);
	void karinAction(unsigned action, bool state);
#endif
...
#ifdef _HARMATTAN_OPTMIZE
idCVar idUsercmdGenLocal::h_cAlwaysRun( "h_cAlwaysRun", "1", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL, "[Harmattan] - Always run (reverse _speed button), in all game mode : 0 / 1" );
#endif
...
idUsercmdGenLocal::idUsercmdGenLocal(void)
{
	...
	toggled_zoom.Clear();
#ifdef _HARMATTAN_OPTMIZE
	toggled_run.on = h_cAlwaysRun.GetBool() || in_alwaysRun.GetBool();
#else
...
#ifdef _HARMATTAN_OPTMIZE
	if ( toggled_run.on ^ ( (h_cAlwaysRun.GetBool()) || ( in_alwaysRun.GetBool() && idAsyncNetwork::IsActive() )) )
#else
...
void idUsercmdGenLocal::InitCurrent(void)
{
	...
	cmd.impulse = impulse;
#ifdef _HARMATTAN_OPTMIZE
	cmd.buttons |= (h_cAlwaysRun.GetBool() || ( in_alwaysRun.GetBool() && idAsyncNetwork::IsActive() )) ? BUTTON_RUN : 0;
#else
...
#ifdef _HARMATTAN_PLUS
void idUsercmdGenLocal::karinAction( unsigned action, bool down ) 
{

	if(action > UB_MAX_BUTTONS)
		return;
...
void idUsercmdGenLocal::karinPollAction(void)
{
	int numEvents = karinPollActionInputEvents();

	if ( numEvents ) {
		unsigned action;
		bool state;
		for( int i = 0; i < numEvents; i++ ) {
			if (karinReturnActionInputEvent( i, action, state )) {
				karinAction ( action, state );
			}
		}
	}

	karinEndActionInputEvents();

	numEvents = karinPollTouchInputEvents();
	if ( numEvents ) {
		for( int i = 0; i < numEvents; i++ ) {
			unsigned action;
			int value;
			if ( karinReturnTouchInputEvent( i, action, value ) ) {
				if ( action >= M_ACTION1 && action <= M_ACTION8 ) {
					mouseButton = K_MOUSE1 + ( action - M_ACTION1 );
					mouseDown = ( value != 0 );
					Key( mouseButton, mouseDown );
