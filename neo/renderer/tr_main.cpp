...
#else
extern void Mesa_glLoadIdentity(float m[16]);
extern void Mesa_glRotate(const float mat[16], float angle, float x, float y, float z, float r[16]);
extern void Mesa_glTranslate(const float mat[16], float x, float y, float z, float m[16]);
	float projectionMatrix[16];
	projectionMatrix[0] = 2 * zNear / height;
	projectionMatrix[4] = 0;
	projectionMatrix[8] = ( ymax + ymin ) / height;	// normally 0
	projectionMatrix[12] = 0;

	projectionMatrix[1] = 0;
	projectionMatrix[5] = 2 * zNear / width;
	projectionMatrix[9] = ( xmax + xmin ) / width;	// normally 0
	...
	Mesa_glRotate(projectionMatrix, -90, 0, 0, 1, tr.viewDef->projectionMatrix);
#endif
}
