...
void idGuiModel::EmitFullScreen(void)
{
	...
	memcpy(projectionMatrix, viewDef->projectionMatrix, sizeof(float) * 16);
	projectionMatrix[0] = 2.0f / 480.0f;
	projectionMatrix[5] = -2.0f / 640.0f;
	projectionMatrix[10] = -2.0f / 1.0f;
	projectionMatrix[12] = -1.0f;
	projectionMatrix[13] = 1.0f;
	projectionMatrix[14] = -1.0f;
	projectionMatrix[15] = 1.0f;
	Mesa_glRotate(projectionMatrix, 90, 0, 0, 1, viewDef->projectionMatrix);
	Mesa_glTranslate(viewDef->projectionMatrix, 0, -480, 0, viewDef->projectionMatrix);
#endif
